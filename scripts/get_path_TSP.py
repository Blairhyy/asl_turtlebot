...
class Navigator_TSP:
    ...
    def __init__(self):
        # rospy.init_node("turtlebot_navigator_TSP", anonymous=True)
        self.mode = Mode.IDLE
...
# <node pkg="asl_turtlebot" type="supervisor.py" name="turtlebot_supervisor" output="screen" />
    ...
    def calculate_path(self, coordinates):
        state_min = self.snap_to_grid((-self.plan_horizon, -self.plan_horizon))
        state_max = self.snap_to_grid((self.plan_horizon, self.plan_horizon))
        sf = 100
        distance_matrix = np.zeros((len(coordinates), len(coordinates)))
        for i, start_pt in enumerate(coordinates):
            for j, end_pt in enumerate(coordinates):
                problem = AStar(
                state_min,
                state_max,
                self.snap_to_grid((start_pt.x, start_pt.y)),
                self.snap_to_grid((end_pt.x, end_pt.y)),
                self.occupancy,
                self.plan_resolution,
            )
                success = problem.solve()
                ...
                # planned_path = problem.path
                path_length = problem.find_best_est_cost_through()
                # print(path_length[0])
                # traj_new, t_new = compute_smoothed_traj(planned_path, self.v_des, self.spline_alpha, self.traj_dt)
                # distance_matrix[i][j] = int(t_new*sf)
                distance_matrix[i][j] = int(path_length[0]*sf)
                
        # Check sf
        print(distance_matrix)
        return distance_matrix

def superviser_get_path(coordinates):
    nav_2 = Navigator_TSP()
    rospy.on_shutdown(nav_2.shutdown_callback)
    return nav_2.calculate_path(coordinates)
