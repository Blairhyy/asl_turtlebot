...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        V_prev_s = self.V_prev

        if V_prev_s < V_PREV_THRES:
            V_prev_s  = np.sqrt(xd_d**2 + yd_d**2)
    

        u = np.array([xdd_d + self.kpx*(x_d - x) + self.kdx*(xd_d - V_prev_s * np.cos(th)),
            ydd_d + self.kpy*(y_d - y) + self.kdy*(yd_d - V_prev_s * np.sin(th))])

        
        J = np.array([[np.cos(th), - V_prev_s *np.sin(th)],
            [np.sin(th), V_prev_s * np.cos(th)]])

        control = np.dot(np.linalg.inv(J), u)
